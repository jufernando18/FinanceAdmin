...
import com.jferza.financeadmin.config.KafkaProperties;
...
class FinanceAdminKafkaResourceIT {
    ...
    void setup() {
        ...
        FinanceAdminKafkaResource kafkaResource = new FinanceAdminKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/finance-admin-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/finance-admin-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
